            ...
            <div className="space-y-6">
                {units.map((unit, index) => {
                    ...
                    return (
                        <div 
                            key={index} 
                            className={`rounded-2xl border-2 overflow-hidden relative transition-all ${
                                isLocked 
                        ...
                        >
                            ...
                            <div className={`h-24 p-6 flex items-start justify-between ${
                                isCompleted ? 'bg-duo-green/20' : (isActive ? 'bg-duo-blue/10' : 'bg-transparent')
                            }`}>
                                <div className="space-y-1 z-10">
                                    <h2 className={`text-2xl font-extrabold ${isCompleted ? 'text-duo-green' : (isActive ? 'text-white' : 'text-duo-muted')}`}>
                                        {unit.title}
                                    </h2>
                                    <p className="text-duo-muted font-bold text-sm max-w-[200px]">{unit.description}</p>
                                </div>
                                {isCompleted && (
