...
import { Home, Shield, User as UserIcon, MoreHorizontal, LogOut, Bot } from 'lucide-react';
...
const LeftSidebar: React.FC<LeftSidebarProps> = ({ user, onLogout, onLearnClick, onProfileClick, onLeaderboardClick, onAITutorClick, currentView }) => {
  ...
  return (
    <div className="hidden lg:flex flex-col w-64 h-screen border-r-2 border-duo-border p-4 fixed left-0 top-0 bg-duo-bg z-10">
      ...
      <nav className="flex-1 space-y-2 overflow-y-auto px-2 custom-scrollbar">
        ...
        {/* AI Tutor for Students Only */}
        {!isTeacher && (
          <NavItem
            icon={Bot}
            label="AI Tutor"
            active={currentView === 'ai-tutor'}
