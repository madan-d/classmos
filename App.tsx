...
const App: React.FC = () => {
    ...
    // View State
    const [currentView, setCurrentView] = useState<'learn' | 'add-course' | 'units-overview' | 'profile' | 'leaderboard' | 'manage-course' | 'course-analytics' | 'ai-tutor'>('learn');
    ...
    useEffect(() => {
        ...
        if (user.role === 'teacher') {
            if (currentView === 'learn' || currentView === 'ai-tutor') {
                setCurrentView('course-analytics');
            }
        } else { // Student
    ...
    return (
        <div className="min-h-screen bg-duo-bg text-white font-sans selection:bg-duo-green selection:text-white">
            ...
            <main className="lg:ml-64 xl:mr-96 min-h-screen pb-20">
                ...
                {/* VIEW: AI TUTOR */}
                {currentView === 'ai-tutor' && user.role === 'student' && (
                    <AITutorScreen courseStructure={courseStructure} />
                )}
